...
class Slider(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        ...
        left_rect = pygame.Rect(self.x, self.y, self.handle_pos - self.x, self.height)
        pygame.draw.rect(screen, self.color, left_rect)

        # Draw the right portion of the slider bar
        right_rect = pygame.Rect(self.handle_pos, self.y, self.width - (self.handle_pos - self.x), self.height)
        pygame.draw.rect(screen, (200, 200, 200), right_rect)
        # Draw handle
        pygame.draw.circle(screen, (self.color), (int(self.handle_pos), self.y + self.height // 2), 10)
